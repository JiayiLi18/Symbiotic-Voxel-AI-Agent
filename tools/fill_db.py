...
class ManualSplitter(RecursiveCharacterTextSplitter):
    def split_documents(self, docs):
        return super().split_documents(docs)  # 所有手册都进行分块
...
def process_documents() -> None:
    ...
    # 初始化文本分割器
    text_splitter = ManualSplitter.from_tiktoken_encoder(
        chunk_size=300,
        chunk_overlap=150,
        separators=["\n\n", "\n", "(?<=。)", " "],
        keep_separator=True
    ...
    # 分割文档
    chunks = text_splitter.split_documents(manuals)
